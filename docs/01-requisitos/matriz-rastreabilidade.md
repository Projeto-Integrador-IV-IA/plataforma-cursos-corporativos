...
| RF13 | Anexar resultado estruturado à negociação | Essencial | ai + pipeline | #37 | 3 | ✅ | `POST /api/v1/demands/{id}/artifacts` persiste em uma transação o bruto do LLM, o estruturado validado, metadados e vínculos N:N com as fontes; `GET` no mesmo caminho recupera tudo pela demanda. Implementação em [`artifact_service.py`](../../services/pipeline-service/app/services/artifact_service.py), integridade em [`artifact.py`](../../services/pipeline-service/app/models/artifact.py) e aceite em [`test_artifacts.py`](../../services/pipeline-service/tests/integration/test_artifacts.py). Corresponde ao RF16.1 do Documento Consolidado v1.0 |
