...
| RF10 | Normalizar a entrada bruta | Alta | ingestion | #33 | 3 | ✅ | Sanitização conservadora em [`text_normalizer.py`](../../services/ingestion-service/app/normalizers/text_normalizer.py), executada somente depois da persistência do original por [`ingestion_service.py`](../../services/ingestion-service/app/services/ingestion_service.py). O resultado é gravado separadamente em `raw_inputs.normalized_content`; testes cobrem ruído de e-mail, WhatsApp, HTML/Markdown, preservação do conteúdo e ordem das escritas. Corresponde ao RF11 do Documento Consolidado v1.0 |
