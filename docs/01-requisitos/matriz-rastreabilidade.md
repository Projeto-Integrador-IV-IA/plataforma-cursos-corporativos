...
|---|---|---|---|---|---|---|---|
| RF01 | Cadastrar/editar/consultar clientes | Essencial | pipeline | #23, #24 | 3 | 🟡 | CRUD em [`routes/clients.py`](../../services/pipeline-service/app/api/v1/routes/clients.py) e [`client_service.py`](../../services/pipeline-service/app/services/client_service.py): `POST`, `GET` (lista paginada), `GET /{id}` e `PATCH`, declarados em [`pipeline-service.yaml`](../../packages/contracts/openapi/pipeline-service.yaml). Erros pela hierarquia `PlatformError` de [`core/exceptions.py`](../../services/pipeline-service/app/core/exceptions.py), registrada em `app/main.py`. Testes em [`test_clients.py`](../../services/pipeline-service/tests/unit/test_clients.py). Remoção/inativação de cliente ainda não exposta |
| RF02 | Cadastrar demandas vinculadas a cliente | Essencial | pipeline | #27, #149 | 3 | ✅ | `POST /api/v1/demands` cria a demanda vinculada; `GET /api/v1/demands/{id}` consulta contexto, cliente, etapa e artefatos; `PATCH /api/v1/demands/{id}` altera somente campos enviados. Implementação em [`routes/demands.py`](../../services/pipeline-service/app/api/v1/routes/demands.py), [`demand_service.py`](../../services/pipeline-service/app/services/demand_service.py) e [`demand_repository.py`](../../services/pipeline-service/app/repositories/demand_repository.py), contrato em [`pipeline-service.yaml`](../../packages/contracts/openapi/pipeline-service.yaml) e aceite em [`test_demands.py`](../../services/pipeline-service/tests/integration/test_demands.py). Exige um cliente já persistido (RF01) |
| RF03 | Listar e filtrar por status, cliente e período | Alta | pipeline + web | #28 | 3 | 🟡 | `GET /api/v1/demands` em [`routes/demands.py`](../../services/pipeline-service/app/api/v1/routes/demands.py) filtra por cliente, etapa, responsável, período e situação, combinados com AND, com paginação `limit`/`offset` e total calculado antes do recorte ([`demand_repository.py`](../../services/pipeline-service/app/repositories/demand_repository.py)). Envelope reutilizável em [`schemas/common.py`](../../services/pipeline-service/app/schemas/common.py). `GET /api/v1/clients` com paginação veio no #182. Telas de listagem (#73, #74) pendentes |
...
| RF11 | Extrair requisitos do texto não estruturado | Essencial | ai-structuring | #5, #6 | 2 (PoC) / 3 | 🟡 | Caso de uso ponta a ponta em [`structuring_service.py`](../../services/ai-structuring-service/app/services/structuring_service.py) (`structure_course`: prompt versionado → provedor → validação de schema), exposto em `POST /api/v1/structuring` ([`routes/structuring.py`](../../services/ai-structuring-service/app/api/v1/routes/structuring.py), DTOs em [`schemas/structuring.py`](../../services/ai-structuring-service/app/schemas/structuring.py)) — card #6, que cita **RF13.1** no Documento Consolidado de Requisitos v1.0. Prompt versionado [`extract-requirements.v1.md`](../../services/ai-structuring-service/app/prompts/extract-requirements.v1.md), carregado por versão em [`app/prompts/__init__.py`](../../services/ai-structuring-service/app/prompts/__init__.py). Testes em [`test_structuring_endpoint.py`](../../services/ai-structuring-service/tests/integration/test_structuring_endpoint.py), [`test_extract_requirements.py`](../../services/ai-structuring-service/tests/integration/test_extract_requirements.py) e [`test_prompts.py`](../../services/ai-structuring-service/tests/unit/test_prompts.py). Falta o prompt pedir `nicho`, `numero_participantes` e `formato`, hoje sempre ausentes na saída |
| RF12 | Gerar ementa com objetivos de aprendizagem | Essencial | ai-structuring | #6 | 2 (PoC) / 3 | 🟡 | Ementa e objetivos atravessam o fluxo validados na forma canônica e saem em `POST /api/v1/structuring` ([`routes/structuring.py`](../../services/ai-structuring-service/app/api/v1/routes/structuring.py)), verificados em [`test_structuring_endpoint.py`](../../services/ai-structuring-service/tests/integration/test_structuring_endpoint.py). Hoje vêm apenas do que o próprio cliente descreveu no texto: a **geração** depende de [`generate-syllabus.v1.md`](../../services/ai-structuring-service/app/prompts/generate-syllabus.v1.md), ainda em rascunho, e segue pendente |
...
| RNF03 | Prompts com schema de saída definido | Essencial | #3, #4, #7, #8 | 2 | 🟡 | Contrato em [`providers/base.py`](../../services/ai-structuring-service/app/providers/base.py), erros tipados em [`core/exceptions.py`](../../services/ai-structuring-service/app/core/exceptions.py) e provedores `mock`/`http` selecionados por `LLM_PROVIDER` ([`providers/factory.py`](../../services/ai-structuring-service/app/providers/factory.py)); testes em [`tests/unit`](../../services/ai-structuring-service/tests/unit). Saída canônica em [`domain/course.py`](../../services/ai-structuring-service/app/domain/course.py) (card #7, RF14.1 no Documento Consolidado v1.0): `StructuredCourse` com chaves, ordem e tipos fixos entre execuções, chave fora do contrato recusada e `validar_curso_estruturado()` levantando `LLMInvalidResponseError`; testes em [`test_course_schema.py`](../../services/ai-structuring-service/tests/unit/test_course_schema.py) e [`test_structured_course_contract.py`](../../services/ai-structuring-service/tests/integration/test_structured_course_contract.py). Política de campo não inferível em [`domain/course.py`](../../services/ai-structuring-service/app/domain/course.py) (card #8, RF14.2 no Documento Consolidado v1.0): campo sem base no texto vira `null` ou lista vazia e entra em `campos_ausentes`, valor devolvido para campo declarado ausente é descartado e o motivo de cada ausência fica em `observacoes` (`campo: motivo`), exposto por `motivos_dos_campos_ausentes()`; prompt [`extract-requirements.v2`](../../services/ai-structuring-service/app/prompts/extract-requirements.v2.md) reforça a proibição do valor plausível; testes em [`test_missing_fields.py`](../../services/ai-structuring-service/tests/unit/test_missing_fields.py). Caso de uso da estruturação e o JSON Schema de `packages/contracts` pendentes |
